...
export default function ReviewCenter() {
  ...
  async function decideReport(
    reportId: string,
    decision: "investigating" | "resolved" | "dismissed" | "takedown",
  ) {
    ...
    <div className="page">
      ...
      {error && (
        <section className="panel">
          ...
          <p className="studioHint">如果你不是审核员或管理员，这是预期的权限拒绝。</p>
        </section>
      )}
      {!error && (
        <div className="stack">
          <section className="panel">
            <h2>待审版本</h2>
            {reviews.length === 0 ? (
              <p className="studioHint">当前没有待审版本。</p>
            ) : (
              reviews.map((item) => (
                <article className="releaseRow" key={item.case_id}>
                  <div>
                    <b>{item.title}</b>
                    <small>
                      {item.rating} · {new Date(item.submitted_at).toLocaleString()}
                    </small>
                    {item.evidence.length > 0 && <small>包含申诉材料</small>}
                  </div>
                  <div>
                    <button
                      className="dangerLink"
                      onClick={() =>
                        decideReview(item.case_id, "approved").catch((exception) =>
                    ...
                    <button
                      className="dangerLink"
                      onClick={() =>
                        decideReview(item.case_id, "rejected").catch((exception) =>
                          setError(exception.message),
                        )
                      }
                    >
                    ...
                    <button
                      className="dangerLink"
                      onClick={() =>
                        decideReview(item.case_id, "withdrawn").catch((exception) =>
                          setError(exception.message),
                        )
                      }
                    >
          ...
          <section className="panel">
            <h2>举报队列</h2>
            {reports.length === 0 ? (
              <p className="studioHint">当前没有待处理举报。</p>
            ) : (
              reports.map((item) => (
                <article className="releaseRow" key={item.id}>
                  ...
                  <div>
                    <button
                      className="dangerLink"
                      onClick={() =>
                        decideReport(item.id, "investigating").catch((exception) =>
                          setError(exception.message),
                        )
                      }
                    >
                    ...
                    <button
                      className="dangerLink"
                      onClick={() =>
                        decideReport(item.id, "dismissed").catch((exception) =>
                          setError(exception.message),
                        )
                      }
                    >
                    ...
                    <button
                      className="dangerLink"
                      onClick={() =>
                        decideReport(item.id, "takedown").catch((exception) =>
                          setError(exception.message),
                        )
                      }
                    >
