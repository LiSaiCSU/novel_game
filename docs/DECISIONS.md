...
## D-002（已被 D-028 取代）前端：V1 用零构建 SPA，保留 Next.js 迁移路径
...
> 2026-08-13：宿主环境已经具备 Node.js，迁移路径已完成；`apps/web/` 已删除，当前前端为 `apps/web-next/`。
...
## D-028 多用户创作平台采用 Next.js 与 Content Pack v2

**背景**
V1 零构建界面、全局内容包和单世界运行时无法表达多用户所有权、不可变版本、题材通用字段与创作者发布工作流。

**决策**

- 玩家端和创作台迁移到 `apps/web-next/`；所有正式功能只使用租户安全的 `/api/v1`。
- Content Pack v2 是作者契约，Release 是带校验和的不可变运行制品，Playthrough 永久固定 Release。
- 普通作者只使用受类型检查的声明式 AST；受信任 Python 插件仅由管理员随部署安装。
- LLM 是可替换的理解与表达层，每次请求按玩家、Playthrough、Release 和凭据构造运行时；模型输出只能提出变更。
- 生产使用 PostgreSQL RLS、Redis 和对象存储；SQLite/内存实现只用于测试和轻量开发。

**后果**
创作者可以在不修改引擎源码的情况下发布新题材，玩家数据和旧存档有清晰的租户与版本边界。代价是部署组件、迁移、审核和运维复杂度上升。
