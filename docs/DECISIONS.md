...
**决策**
- 叙事模型一次产出「正文 + `---BEAT---` + 紧凑 JSON」：正文是 300—600 字的完整场景，
...
- **一次规划、逐步裁决**：`Autopilot.plan_run()` 一次调用规划最多 N 步，
  ...
  中断发生时，计划中剩余的步骤直接作废。
- **一次叙事**：整段跑完后由 `ChapterRenderer` 写成**一章**（600—1000 字），
...
- **章节目标长度 600—1000 字 → 1500—2500 字**，`auto_advance.max_steps` 5 → 8。
...
- **章节改为真流式**：`ChapterRenderer` 走 `stream_text`，边写边推给前端，
  并在遇到 `---BEAT---` 前扣住一个标记长度的尾巴，保证结构化块不会泄漏进正文。

**后果**
玩家一次输入换来的是一章可读的小说，而不是一小格进度；他说的话不会再被覆盖。
代价是单次请求更长（覆盖至多 8 步、写 1500—2500 字），上下文与输出预算显著上升。
